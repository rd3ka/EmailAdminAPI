...
//import com.yet.another.Password.EncryptionUtil;
...
public class Password {
	...
	private final String GeneratePassword() {
		...
		/*
		 * The first step is to set the password length by default, the password length
		 * can be any value between 6 and 12
		 * 
		 * In case of custom lower and upper limit values, call the setLength func again
		 * with a @param
		 */
		this.setLength();
	...
	/*
	 * checks the strength of the password, internally if the password is low, the
	 * password cannot be set by the user else if the password is med or high, the
	 * password will get set
	 * 
	 * TO-DO: Needs logic improvement!
	 */
